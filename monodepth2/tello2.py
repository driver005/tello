    ...
    if(max > 0.5):
        ...
        for y in range(0, original_height - size - 1, step_size):
            for x in range(0, original_width - size - 1, step_size):
                ...
                averages.append(np.average(part_img))
                img_parts.append((x + (size / 2), y + (size / 2)))

        averages = np.array(averages)
        data = np.argsort(averages)[:2]

        for d in data:
            passed_cubes.append(img_parts[d])

        print(np.average(passed_cubes))
        ...
        if(len(passed_cubes) == 0):
            ...
            #dtello.send_rc_control(0, 20, 0, 0)
